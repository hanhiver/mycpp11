...
int main(int argc, char* argv[])
{
    ...
    std::cout << "SURF total time: " << sum_match_time/1000.0/1000.0 << "s, "
    ...
    for (auto& item : file_list)
    {
        ...
        else
        {
            ...
            for (int i=0; i<img_queue.size(); i+=2)
            {
                double res = hist_comp(img_queue[i], img_queue[i+1]);
    ...
    std::cout << "HIST total time: " << sum_match_time/1000.0/1000.0 << "s, "
              << "avg per frame: " << sum_match_time/double(time_static.size()) << "us. \n"
              << std::endl; 
    ...
    for (auto& item : file_list)
    {
        ...
        else
        {
            ...
            for (int i=0; i<img_queue.size(); i+=2)
            {
                double res = combine(img_queue[i], img_queue[i+1]);
                match_rates.push_back(res);
    ...
    std::cout << "COMB total time: " << sum_match_time/1000.0/1000.0 << "s, "
              << "avg per frame: " << sum_match_time/double(time_static.size()) << "us. \n"
              << std::endl; 
